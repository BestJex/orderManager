...
public class MenuController {
    ...
    @Resource
    private RolemenuService trolemenuService;
    ...
    */
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public ResponseMessage selectMenuById(Integer id) {
        try {
            if(id==null||id==0){
                return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"无法获取节点id");
            }else{
                Menu menu = menuService.selectByKey(id);
                if(menu == null){
                    return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"无法找到该节点对象");
                }else{
                    return ResponseMessage.initializeSuccessMessage(menu);
                }
            }
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public ResponseMessage addupdatemenu(Menu tmenu) {
        try {
            if (tmenu.getId() == null) {//新建
                ...
                if (menulist != null && menulist.size() > 0) {
                    return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"当前菜单名已存在");
                }

                //校验是否提交了pId
                if(tmenu.getpId()==null||tmenu.getpId()==0){
                    return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"无法获取父级id");
                }else{

                    Menu pmenu=menuService.selectByKey(tmenu.getpId());//父节点对象
                    if(pmenu.getState()==3){
                        return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"3级菜单不可再添加子菜单");
                    }
                    if("-1".equalsIgnoreCase(String.valueOf(pmenu.getpId()))
                        ...
                        if(isNumeric(tmenu.getName())){
                            return new ResponseMessage(ResponseMessage.PARAMETER_ERROR,"1级菜单的名字不可为纯数字");
                        }
            ...
            return ResponseMessage.initializeSuccessMessage( String.valueOf(tmenu.getId()));
        } catch (Exception e) {
            return new ResponseMessage(ResponseMessage.SYSTEM_ERROR,"操作失败，系统异常");
        }
    }
    ...
    @RequiresPermissions(value = {"菜单管理"})
    public ResponseMessage deleteMenu(Menu tmenu) {
        try {
            if(tmenu.getId()!=null&&!tmenu.getId().equals(0)){
                Menu menu = menuService.selectByKey(tmenu.getId());
                if(menu == null){
                    ...
                    if(true){
                        ...
                        menuService.deleteByExample(tmenuexample);
                    }
                    //删除该节点
                ...
                return new ResponseMessage(ResponseMessage.PARAMETER_ERROR, "删除失败");
            }
